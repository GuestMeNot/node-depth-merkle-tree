...
mod tests {
    ...
    use crate::{
        Blake3MerkleTreeHasher, BlakeMerkleTree, MerkleTree, MerkleTreeHasher, Sha256MerkleTree,
    };
    ...
    fn blake3_merkle_tree_serde_test() {
        ...
        let tree_de: MerkleTree<[u8; 32], Blake3MerkleTreeHasher> =
    ...
    fn blake3_merkle_tree_serde_failed_test() {
        ...
        let tree_de1: MerkleTree<[u8; 32], Blake3MerkleTreeHasher> =
            serde_json::from_str(&json1).unwrap();

        let tree2 =
            BlakeMerkleTree::build(&blake3_hash_leaf_values(&SINGLE_CHAR_VALUES_DUP_LAST_NODE))
                .unwrap();
        let json2 = serde_json::to_string(&tree2).unwrap();
        let tree_de2: MerkleTree<[u8; 32], Blake3MerkleTreeHasher> =
    ...
    fn thwart_second_image_attack_using_interior_nodes() {
        let valid_tree =
            BlakeMerkleTree::build(&blake3_hash_leaf_values(&SINGLE_CHAR_VALUES)).unwrap();
