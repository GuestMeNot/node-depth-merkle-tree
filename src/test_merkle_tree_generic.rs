...
{
    let tree = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
...
{
    let tree1 = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
    let json1 = serde_json::to_string(&tree1).unwrap();
    let tree_de1: MerkleTree<T, H> = serde_json::from_str(&json1).unwrap();

    let tree2 = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES_DUP_LAST_NODE)).unwrap();
...
#[cfg(any(test))]
#[doc(hidden)]
pub(crate) fn merkle_root_calculation_test<T, H>(hash: &dyn Fn(&[&str]) -> Vec<T>, expected_root: &str)
where
...
{
    let tree1 = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
    let tree2 = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
...
#[cfg(any(test))]
#[doc(hidden)]
pub(crate) fn thwart_second_image_attack_using_interior_nodes<T, H>(hash: &dyn Fn(&[&str]) -> Vec<T>)
where
    T: AsRef<[u8]> + Copy + Debug + PartialEq + Sized,
    H: Debug + Default + MerkleTreeHasher<T>,
{
    let valid_tree = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
    ...
    let attack_tree = MerkleTree::<T, H>::build(&attack_leaves).unwrap();
    ...
    H: Debug + Default + MerkleTreeHasher<T>,
{
    ...
    let attack_tree: MerkleTree<T, H> = MerkleTree::build(&attack_leaves).unwrap();
...
#[cfg(any(test))]
#[doc(hidden)]
pub(crate) fn thwart_second_image_attack_using_duplicate_odd_node<T, H>(hash: &dyn Fn(&[&str]) -> Vec<T>)
where
    T: AsRef<[u8]> + Copy + Debug + PartialEq + Sized,
    H: Debug + Default + MerkleTreeHasher<T>,
{
    let valid_tree = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES)).unwrap();
    let attack_tree = MerkleTree::<T, H>::build(&hash(&SINGLE_CHAR_VALUES_DUP_LAST_NODE)).unwrap();
